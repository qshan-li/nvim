...
vim.api.nvim_create_user_command("FormatterToggleFt", function(opts)
	if formatter_block_list[opts.args] == nil then
		...
		vim.notify(
			...
			{ title = string.format("LSP Formatter %s", not formatter_block_list[opts.args] and "Info" or "Warning") }
		)
	end
...
function M.format_filter(clients)
	return vim.tbl_filter(function(client)
		...
		-- Prefer none-ls when it is attached so external formatters stay authoritative.
...
function M.format(opts)
	...
	for _, client in pairs(clients) do
		if formatter_block_list[vim.bo.filetype] == true then
			vim.notify(
				string.format(
