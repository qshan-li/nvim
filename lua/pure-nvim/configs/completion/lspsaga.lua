return function()
	require("pure-nvim.utils").gen_lspkind_hl()

	local icons = {
		cmp = require("pure-nvim.utils.icons").get("cmp", true),
		diagnostics = require("pure-nvim.utils.icons").get("diagnostics", true),
		kind = require("pure-nvim.utils.icons").get("kind", true),
		type = require("pure-nvim.utils.icons").get("type", true),
	...
	require("pure-nvim.utils").load_plugin("lspsaga", {
		...
		hover = {
			...
			max_height = 0.7,
			open_link = "gl",
			open_cmd = "silent !" .. require("pure-nvim.core.settings").external_browser,
			silent = true,
		},
	...
	-- 确保 code_action 窗口响应 Esc 和 Ctrl+C
	vim.api.nvim_create_autocmd("FileType", {
		pattern = "markdown",
		callback = function(args)
			-- 检查是否是 lspsaga code_action 窗口
			local bufname = vim.api.nvim_buf_get_name(args.buf)
			if bufname == "" or vim.bo[args.buf].buftype == "nofile" then
				-- 延迟执行，确保不与 lspsaga 的映射冲突
